...
resource "google_compute_address" "mariadb" {
  ...
  subnetwork   = "projects/${var.network_project}/regions/${lookup(var.region, count.index)}/subnetworks/${lookup(var.subnetwork, count.index)}"
...
data "template_file" "startup" {
  ...
  vars = {
    script_path = "gs://${var.bucket_name}/${google_storage_bucket_object.config.name}"
  }
}
...
resource "google_compute_instance_group_manager" "mariadb" {
  ...
  auto_healing_policies {
    health_check      = "projects/${var.project_id}/global/healthChecks/${var.health_check_name}"
...
data "template_file" "garb_startup" {
  template = "${file("${path.module}/scripts/startup.sh.tpl")}"
  vars = {
    script_path = "gs://${var.bucket_name}/${google_storage_bucket_object.garb.name}"
  }
}
...
resource "google_compute_instance_template" "garb" {
  ...
  network_interface {
    subnetwork = "projects/${var.network_project}/regions/${var.garb_region}/subnetworks/${var.garb_subnetwork}"
  }
  service_account {
    email  = "${var.service_account}@${var.project_id}.iam.gserviceaccount.com"
    scopes = ["cloud-platform"]
